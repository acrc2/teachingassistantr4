...
defineSupportCode(function ({ Given, When, Then }) {
    Given(/^The student "([^\"]*)" with CPF "(\d*)" is registered in the system$/, async (name, cpf) => {
    ...
    When(/^I delete the student "([^\"]*)" with CPF "(\d*)"$/, async (name, cpf) => {
        await $("input[name='namebox']").sendKeys(<string> name);
        await $("input[name='cpfbox']").sendKeys(<string> cpf);
    ...
    Then(/^the student "([^\"]*)" with CPF "(\d*)" is not in the system anymore$/, async (name, cpf) => {
    ...
    Then(/^the student "([^\"]*)" with CPF "(\d*)" is still in the system$/, async (name, cpf) => {
        var allalunos : ElementArrayFinder = element.all(by.name('alunolist'));
